...
<template>
  <article class="glass-panel overflow-hidden flex flex-col cursor-pointer" @click="emit('select', game)">
    <!-- Spotlight Banner -->
    <div v-if="pAwayName || pHomeName" class="bg-zinc-900 border-b border-accent-base/20 px-4 py-3 flex items-center justify-center shadow-inner">
      <div class="flex-1 text-right">
        <span v-if="pAwayName" class="text-sm sm:text-base font-black uppercase tracking-widest drop-shadow-md" :class="statusState === 'post' ? (awayTeam.winner ? 'text-emerald-400' : 'text-zinc-500') : 'text-accent-base'">{{ pAwayName }}</span>
      ...
      <div class="flex-1">
        <span v-if="pHomeName" class="text-sm sm:text-base font-black uppercase tracking-widest drop-shadow-md" :class="statusState === 'post' ? (homeTeam.winner ? 'text-emerald-400' : 'text-zinc-500') : 'text-accent-base'">{{ pHomeName }}</span>
        <span v-else class="text-[0.7rem] font-medium text-zinc-600 uppercase tracking-wider">Unowned</span>
      </div>
    </div>
    ...
    <div class="p-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-4 flex-1">
      ...
      <!-- Away Team -->
      <div class="flex flex-col items-center text-center gap-1 cursor-pointer group" @click.stop="goToTeam(awayTeam.team?.id)">
         <div class="relative inline-block group-hover:scale-110 transition-transform duration-200">
            <img v-if="awayTeam.team?.logo" :src="awayTeam.team.logo" class="w-10 h-10 sm:w-12 sm:h-12 object-contain drop-shadow-md relative z-10 transition-opacity duration-300" :class="statusState === 'post' && awayTeam.winner === false ? 'opacity-40' : 'opacity-100'" />
         ...
         <div class="flex flex-col mt-1">
           <span class="font-semibold text-sm leading-tight transition-colors duration-300" :class="statusState === 'post' && awayTeam.winner === false ? 'text-zinc-500' : 'text-zinc-200'">{{ awayTeam.team?.shortDisplayName || awayTeam.team?.name }}</span>
         </div>
      ...
      <div class="flex flex-col items-center justify-center text-center px-2 min-w-[70px]">
         ...
         <template v-else-if="statusState === 'post'">
            <span class="text-white font-bold tracking-widest text-sm">FINAL</span>
         </template>
      </div>

      <!-- Home Team -->
      <div class="flex flex-col items-center text-center gap-1 cursor-pointer group" @click.stop="goToTeam(homeTeam.team?.id)">
         <div class="relative inline-block group-hover:scale-110 transition-transform duration-200">
            <img v-if="homeTeam.team?.logo" :src="homeTeam.team.logo" class="w-10 h-10 sm:w-12 sm:h-12 object-contain drop-shadow-md relative z-10 transition-opacity duration-300" :class="statusState === 'post' && homeTeam.winner === false ? 'opacity-40' : 'opacity-100'" />
         ...
         <div class="flex flex-col mt-1">
           <span class="font-semibold text-sm leading-tight transition-colors duration-300" :class="statusState === 'post' && homeTeam.winner === false ? 'text-zinc-500' : 'text-zinc-200'">{{ homeTeam.team?.shortDisplayName || homeTeam.team?.name }}</span>
         </div>
